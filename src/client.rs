...
impl ClientBuilder {
    pub fn build(&mut self) -> Result<Client> {
        ...
        // let user_agent = self.user_agent.as_ref().unwrap_or(&"kimchi/0.3.0".to_string());
        let user_agent = match self.user_agent {
            Some(ref u) => u.clone(),
            None => String::from("kimchi/0.3.0"),
        };
...
mod test {
    ...
    async fn test_exponential_backoff() {
        ...
        let uri = Uri::Website(Url::parse("https://mechanikadesign.com/abcd").unwrap());
    ...
    async fn test_github() {
        assert!(matches!(
            ClientBuilder::default()
                .build()
                .unwrap()
                .check(website_url("https://github.com/wgalyen/codesweeper"))
                .await
                .status,
            Status::Ok(_)
        ));
    ...
    async fn test_github_nonexistent() {
        let res = ClientBuilder::default()
            ...
            .unwrap()
            .check(website_url(
                "https://github.com/wgalyen/doesnt-exist-man",
            ))
            .await
            .status;
    ...
    async fn test_non_github() {
        let res = ClientBuilder::default()
            .build()
            .unwrap()
            .check(website_url("https://mechanikadesign.com"))
            .await
            .status;
