...
impl ClientBuilder {
    ...
    pub fn build(&mut self) -> Result<Client> {
        ...
        let user_agent = self
            .user_agent
            .clone()
            .unwrap_or_else(|| format!("kimchi/{}", VERSION));
...
mod test {
    ...
    fn test_is_github() {
        assert_eq!(
            ClientBuilder::default()
                .build()
                .unwrap()
                .extract_github("https://github.com/wgalyen/kimchi")
                .unwrap(),
            ("wgalyen".into(), "kimchi".into())
        );
    }
    #[tokio::test]
    async fn test_github() {
        assert!(matches!(
            ClientBuilder::default()
                .build()
                .unwrap()
                .check(website_url("https://github.com/wgalyen/kimchi"))
                .await
                .status,
            Status::Ok(_)
        ));
    ...
    async fn test_github_nonexistent() {
        let res = ClientBuilder::default()
            .build()
            .unwrap()
            .check(website_url("https://github.com/wgalyen/not-kimchi"))
            .await
            .status;
    ...
    async fn test_custom_headers() {
        let res = ClientBuilder::default()
            .build()
            .unwrap()
            .check(website_url("https://crates.io/crates/wgalyen"))
            .await;
