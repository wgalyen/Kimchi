...
mod test {
    ...
    fn test_extract_markdown_links() {
        let input = "This is [a test](https://mechanikadesign.com).";
        let links = extract_links(input);
        assert_eq!(
            links,
            HashSet::from_iter([Url::parse("https://mechanikadesign.com").unwrap()].iter().cloned())
        )
    }
    ...
    fn test_non_markdown_links() {
        let input = "https://mechanikadesign.com and https://wgalyen.show/foo/bar?lol=1";
        let links = extract_links(input);
        let expected = HashSet::from_iter(
            [
                Url::parse("https://mechanikadesign.com").unwrap(),
                Url::parse("https://wgalyen.show/foo/bar?lol=1").unwrap(),
            ]
            .iter()
            .cloned(),
        );
