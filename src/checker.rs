...
use crate::options::KimchiOptions;
use anyhow::anyhow;
...
impl Excludes {
    pub fn from_options(options: &KimchiOptions) -> Self {
        Self {
...
mod test {
    ...
    async fn test_nonexistent() {
        let res = get_checker(false, HeaderMap::new())
            .check(&Uri::Website(
                Url::parse("https://mechanikadesign.com/abcd").unwrap(),
            ))
            .await;
    ...
    async fn test_github() {
        assert!(matches!(
            get_checker(false, HeaderMap::new())
                .check(&Uri::Website(
                    Url::parse("https://github.com/wgalyen/codesweeper").unwrap()
                ))
                .await,
            Status::Ok(_)
        ));
    ...
    async fn test_non_github() {
        let res = get_checker(false, HeaderMap::new())
            .check(&Uri::Website(Url::parse("https://mechanikadesign.com").unwrap()))
            .await;
